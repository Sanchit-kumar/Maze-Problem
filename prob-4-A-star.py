...
#Function to check the valid neighbour  (can't comment above few lines of  function, so adding new function" 
def check_position(row, col, n, maze):
    if row in range(n) and col in range(n) and maze[row][col] == 0:
        return 1
    return 0 
def manhattan(node,end,n): #manhattan heuristic
	rs=node//n
	rd=end//n
	cs=node%n
	cd=end%n
	return (abs(rd-rs)+abs(cd-cs))
...
def search_algo(n, maze, start, end):
    ...
    #(f(x),start,end,move,cost)
    pq.put((manhattan(start,end,n),pos,"start",pos,0))  #putting cost (hx) ,position(current), move, position(parent), path_cost   
    while not pq.empty():
# =============================================================================        
        item=pq.get()
        pos=item[1];
        #cost=item[0] 				#new f(x) parent
        cost_hx=manhattan(pos,end,n) 		#h(parent)
        cost_gx=item[0]-cost_hx 		#g(x)=f(parent_node)-h(parent_node)      
       	explored_cost+=item[4]
        #FORMULA USED
        #f(x)=g(x)+h(x)
        #g(x)= path_cost + g(parent_node)
        #g(parent_node)=f(parent_node)-h(parent_node)
        
        if pos==end:
            print("From %d -> %d"%(item[3],pos),"Optimal cost(g(x)):",cost_gx,"Move:",item[2],"Explored cost:",explored_cost)
            global start_time
        ...
        if check_position(row-1, col, n, maze)==1: #checking up position is valid or not
            ...
            new_cost=cost_gx+2+manhattan(new_pos,end,n)  #new node cost=g(x) of parent node + distance from parent node + h(X) of child
            pq.put((new_cost,new_pos,"U",pos,2)) #puttin the found item to the priority queue
                 
        if check_position(row, col-1, n, maze)==1: #checking left position is valid or not
            maze[row][col-1]=-1
            new_pos=n*(row)+col-1
            new_cost=cost_gx+2+manhattan(new_pos,end,n)
        ...
        if check_position(row+1, col, n, maze)==1: #checking down position is valid or not
            maze[row+1][col]=-1
            new_pos=(n*(row+1)+col)
            new_cost=cost_gx+3+manhattan(new_pos,end,n)
...
# =============================================================================
    ...
    print("Total cost (for possible optimal path cost):",cost_gx)
    print("Total explored cost (Assuming cost of 3 for down movement, and 2 for rest) :",explored_cost)
    print("Note:-- Green boxes is the path taken by A* algo ")
